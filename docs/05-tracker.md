...
| T-203 | FastAPI: /score, /score/{id}, /explain/{id}, /portfolio, /consent, /msmes, /simulate + OpenAPI. **Jul 9 consistency fix: `/portfolio` scored the FULL record and cached it forever, so a killed source left the lender book asserting High confidence while the card had gone provisional (and the alert's trend tail contradicted the gauge beside it: 595→536 vs 534). The book now scores through `AdapterRegistry.visible()` — the same coverage the card sees — and the cache is keyed on the kill set so flipping a switch rebuilds it. Two regression tests pin the invariant (book==card under kill-switch; book survives losing the trend source).** | FR-3.1/3.2 | CL+ZS | **done** |
| T-204 | Audit log + consent store (SQLite; version stamps per row) | FR-3.3 | CL | **done** |
| T-205 | Parakh Watch: score time-series (rolling monthly) + transparent alert rules. **Jul 9: trend + alerts now computed on the coverage-masked record, so the sparkline's last point always equals the card's score. Killing the bank feed empties the series → book returns empty trend, no alerts, HTTP 200 (truthful: with `aa_deposit` dark we cannot assert a trend).** | FR-1.5/1.6 | ZS | **done** |
...
| T-214 | Seal-dot coverage ring + assay-stamp A–E chips. **Jul 9 readability fix (JR): disabled/off source seals were dim red-maroon at 35% opacity → unreadable & looked like errors. Removed `opacity-35`; now full-opacity, state shown by colour+border — live=teal solid, off/killed=light-slate grey dashed, genuine-fail=readable soft red (`SourceSeals` in `bits.jsx`). Readable in BOTH states. Jul 9 (2): the kill-switch marks a source FAILED, so killed seals still painted dim red and read as errors → an off seal now renders legible grey whatever the cause (fact, not error); the *why* stays in the tooltip + fetch banner + coverage caveat. Jul 9 (3): `#C0392B` is only ~2.3:1 on navy — fine as a thick gauge arc, unreadable as text → new `band.red-lit` (#F2897C, ~6.5:1) + shared `RED_ON_DARK` const for the alert banner, Parakh Watch label, "Working against" column, grade chip E, falling sparkline, and the collection screen's unreachable seal (which was also `opacity-50`). Paper surfaces keep the deep band red.** | FR-4.9 | FE+CL | **done** |
...
| T-305 | README rewritten (verified stats, Docker+bare quickstart, architecture, MIT LICENSE) + no-secrets scan clean; **demo GIF pending (after T-308 video)**. **⚠️ Jul 9: CI is RED — not a code failure. Setting the repo private flipped the Actions policy to `allowed_actions: local_only`, so `actions/checkout@v4` / `setup-python@v5` / `setup-node@v4` are refused and every job dies at 0s (`startup_failure`, run 29004078757). Tests themselves are green: 22/22 in-container. Fix needs a repo-settings change (JR): Settings → Actions → General → allow GitHub-authored actions, then `gh run rerun 29004078757`. Must be green before the repo goes public at submit-time — a judge would see a red X on the latest commit.** | NFR-3/7, G1c | NP+CL+JR | doing |
...
| T-318 | QR **created** on the Links slide — currently → public GitHub repo (safe, always-live placeholder). **Swap to live app URL (or 3-min demo video) once hosting up — JR will signal; ~10-sec regen + re-export.** ⚠️ **The QR now points at a PRIVATE repo → it 404s for a judge. Must be swapped before submit, not optional.** **Also recapture the deck's embedded screenshots in the same pass — they predate the Jul 9 readability fixes (dim seals, muddy gauge, unreadable alert bar) and no longer match the app.** | pre-mortem | CL | doing |
